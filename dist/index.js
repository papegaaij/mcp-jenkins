...
import { logger, McpError, loadAllJenkinsInstances, loadToolFilter, } from "./common/index.js";
...
const instanceProperty = {
    instance: {
        type: "string",
        description: "Jenkins instance name (optional — defaults to first configured instance)",
    },
};
const injectInstance = (tool) => ({
    ...tool,
    inputSchema: {
        ...tool.inputSchema,
        properties: {
...
const rawTools = [
    {
        ...
        description: "List all configured Jenkins instances with their names and URLs",
    ...
    },
    {
        ...
        description: "Get the parameter definitions for a parameterised Jenkins job — names, types, defaults, and choices",
        inputSchema: {
            type: "object",
            properties: {
                jobName: {
    ...
    },
    {
        ...
        inputSchema: {
            ...
            properties: {
                ...
                includePassing: {
                    type: "boolean",
                    description: "Include passing/fixed test cases in the suites array (default: false). " +
    ...
    },
    {
        ...
        description: "Fetch a single test case's full detail (status, duration, errorDetails, " +
        ...
        inputSchema: {
            ...
            properties: {
                ...
                className: {
                    type: "string",
                    description: "Fully-qualified class name of the test case " +
                        "(e.g. \"com.acme.FooTest\"), as reported by jenkins_get_test_results.",
                },
                caseName: {
                    type: "string",
                    description: "Test case (method or Spock feature) name, exactly as reported by " +
    ...
    },
    {
        ...
        inputSchema: {
            ...
            properties: {
                nodeName: {
                    type: "string",
                    description: "Node/agent name (use 'master' or 'Built-In Node' for the controller)",
                },
            },
            required: ["nodeName"],
        },
    ...
    },
    {
        ...
        description: "Put Jenkins into quiet mode — no new builds will start until cancelled (requires confirm: true)",
        inputSchema: {
            type: "object",
            properties: {
                reason: {
    ...
    },
    {
        ...
        description: "Safely restart Jenkins — waits for running builds to finish before restarting (requires confirm: true)",
        inputSchema: {
            type: "object",
            properties: {
                confirm: {
...
if (allowlist && blocklist.length) {
    logger.warn("Both JENKINS_TOOLS and JENKINS_BLOCK_TOOLS are set — JENKINS_BLOCK_TOOLS will be ignored");
}
...
try {
    ...
    for (const [name, env] of instances) {
        ...
        clients.set(name, new JenkinsClient({ baseUrl: env.JENKINS_URL, authHeader }));
    ...
    defaultInstance = instances.keys().next().value;
}
catch (error) {
    logger.error("Failed to initialize Jenkins clients", { error: error.message });
    process.exit(1);
}
const resolveClient = (instance) => {
    const name = instance ?? defaultInstance;
    const c = clients.get(name);
    if (!c)
        throw new McpError("INVALID_PARAMS", `Unknown instance "${name}". Available: ${Array.from(clients.keys()).join(", ")}`, 400);
    return c;
};
        ...
        if (name === "jenkins_list_instances") {
            return {
                content: [
                    {
                        type: "text",
                        text: JSON.stringify(Array.from(clients.entries()).map(([instanceName, c]) => ({
                            name: instanceName,
                            url: c.baseUrl,
                        })), null, 2),
                    },
